...
const __checkHasOwnProperty = (objectToCheck, key) => Object.prototype.hasOwnProperty.call(objectToCheck, key)
const __replaceId = (match) => {
    if ((typeof match === 'object') && __checkHasOwnProperty(match, '_id')) {
        try {
            if ((match._id) && (typeof match._id === 'string')) {
                ...
                if (typeof match._id['$eq'] === 'string') {
                    match._id['$eq'] = new ObjectId(match._id['$eq'])
                }
            }
        } catch (ex) {
            match._id = null
...
const __removeAttribute = (doc, attrName) => {
    if ((typeof doc === 'object') && __checkHasOwnProperty(doc, attrName)) {
        try {
            delete doc[attrName]
        } catch (err) {
            logger.warn(err)
    ...
    } else if (__checkHasOwnProperty(doc, '$set') && (doc['$set'][attrName])) {
        try {
